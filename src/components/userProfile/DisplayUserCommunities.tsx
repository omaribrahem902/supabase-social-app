...
async function fetchUserCommunities(userId: string) : Promise<Community[]> {
    const {data, error} = await supabase.from('Communities').select('id,user_id, name, description').eq('user_id', userId).order('created_at', { ascending: false });
    if (error) throw new Error(error.message);
    return data as Community[];
}

export const DisplayUserCommunities = ({userId}: Props) => {
