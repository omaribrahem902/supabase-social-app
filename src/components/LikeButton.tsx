...
export const LikeButton = ({ postId }: Props) => {
  ...
  const { mutate } = useMutation({
    ...
    onMutate: async (voteValue: number) => {
      ...
      queryClient.setQueryData<Vote[]>(["votes", postId], (oldVotes) => {
        if (!oldVotes)
          return [{ post_id: postId, user_id: user!.id, vote: voteValue }];
        ...
        if (existingIndex === -1) {
          ...
          return [
            ...oldVotes,
            { post_id: postId, user_id: user!.id, vote: voteValue },
          ];
        } else {
  ...
  const userVote = votes?.find(
    (v) => v.user_id === user?.id && !("deleted" in v),
  )?.vote;

  return (
    <>
      <div className="rounded-full p-[1px] bg-gradient-to-r from-purple-500 to-blue-500">
        <div className="bg-[#0B1120] px-12 lg:px-14 py-2 rounded-full">
          <button
            onClick={() => mutate(1)}
            className={`flex items-center gap-2 px-3 py-1 rounded text-white transition-colors duration-150 cursor-pointer ${userVote === 1 ? "bg-blue-500" : ""}`}
          >
      ...
      <div className="rounded-full p-[1px] bg-gradient-to-r from-purple-500 to-blue-500">
        <div className="bg-[#0B1120] px-12 lg:px-14 py-2 rounded-full">
          <button
            onClick={() => mutate(-1)}
            className={`flex items-center gap-2 px-3 py-1 rounded text-white transition-colors duration-150 cursor-pointer ${userVote === -1 ? "bg-red-500" : ""}`}
          >
